...
class AnimalActor(val locationManagerActor: ActorRef) extends Actor {
  ...
  def receive = {
    case Tick => locationManagerActor!GetSouroundingRequest(50)
    case GetSouroundingResponse(location: Map[ActorRef, Int], actors: Map[ActorRef, Int]) => {
       ...
       if(!actors.isEmpty){
          ...
          sender!MoveTowardActor(groupedActors.toList.sortBy(_._2.size).iterator.next._2.iterator.next)  
        }else{
          randLoc
        }
      }
